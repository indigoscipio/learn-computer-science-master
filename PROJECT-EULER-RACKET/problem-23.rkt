...
(define (non-abundant-sums n)
  ; maybe we can custom recursive with member here, but i think that would take too long, not sure
  ; maybe even simple HOF, filter -> sum or even vector i dont know
  (let ((abundant-nums (generate-abundant-nums-up-to n))
        (nums (range 1 n))
        )
    (foldr + 0 (filter (λ (n) (not (is-sum-of-two-abundant-nums? n abundant-nums)) ) nums))
    )
  )
(non-abundant-sums 12) ;66
