...
; given a number n, checks if it's pandigital
; maybe i can use member here, but its abit slow
; or faster maybe can use like make-vector then initialize #f for each digit
; dont know what you call that technique but lets try memebr first
(define (is-pandigital? num-digits)
  (define (loop curr-digits digits-seen)
    (cond [(>= (length digits-seen) 9) #t] ; base case
          [(member (car curr-digits) digits-seen) #f] ;already saw in digits seen, false
          [else (loop (cdr curr-digits) (cons (car curr-digits) digits-seen) ) ]
          ; keep searching, cdr digits-seen and cdr curr digits on nextrec
          )
    )
...
(define (pandigital-multiples)
  ...
  ; 1d : 1 - 9, n = 9
  ; 3d: 100 - 999, n = 3
  ; filter the list of numbers and extract the highest
