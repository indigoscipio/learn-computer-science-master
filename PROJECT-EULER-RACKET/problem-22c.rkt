...
(define (sum-name-scores input)
  (let* ((names (map (λ (str) (string-replace str "\"" "")) (string-split input ",")))
    ...
    (define (sum-all xs curr-pos result)
      (cond [(null? xs) result]
            [else (let ((curr-name (car xs)))
                    (sum-all (cdr xs) (+ 1 curr-pos) (+ (* (name-score curr-name) (+ curr-pos 1)) result))
                    )])
      )

    (sum-all sorted-names 0 0)
    )
  )
(sum-name-scores NAMES-SM) ;small list returns 38101
(sum-name-scores NAMES) ;returns 871198282
