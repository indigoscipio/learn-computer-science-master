...
(define (is-truncatable-prime? n)
  (let ((num-digits (num->digits n)))
    ...
    ; truncatable left check
    ; list-of-number -> boolean
    (define (is-truncatable-prime-ltr? xs)
      (cond [(null? xs) #t]
            [else (let* ((rest-xs (cdr xs))
                         (curr-num (digits->number xs)))
                    (and (is-prime? curr-num) (is-truncatable-prime-ltr? rest-xs))
                    )]
            )
      )
  
    ; list-of-number -> boolean
    ; reverse -> cdr ->  reverse again
    (define (is-truncatable-prime-rtl? xs)
      (cond [(null? xs) #t]
            [else (let* ((rest-xs (cdr (reverse xs)))
                         (curr-num (digits->number xs)))
                    (and (is-prime? curr-num)
                         (is-truncatable-prime-rtl? (reverse rest-xs)))
                    )]
            )
      )
    
    ; combine
    (and (is-truncatable-prime-ltr? num-digits)
         (is-truncatable-prime-rtl? num-digits) )
    )
  )
...
(define (truncatable-primes)
  ...
  (define (helper curr-n counter result)
    ; base: counter reaches 11, stop
    ; for each curr n, check if its truncable prime,
    ; if so, increase counter and add sum to result
    ; else keep searching
  ...
  ; start with 11, 2,3,5,7 is not considred truncatble
  (helper 11 0 0)
  )
(truncatable-primes)
