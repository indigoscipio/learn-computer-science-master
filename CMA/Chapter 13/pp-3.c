...
int main(void){
    ...
    for (;;) {
        ...
        if(month<0 || month>12){
            printf("Error: month must be posint <=12\n");
            read_line(msg_str, MSG_LEN);
            continue;
        }
        ...
        char month_day_time_str[11];
        strcpy(month_day_time_str, month_str);
        strcat(month_day_time_str, day_str);
        strcat(month_day_time_str, time_str);
        for (i = 0; i < num_remind; i++)
            if (strcmp(month_day_time_str, reminders[i]) < 0)
                break;
