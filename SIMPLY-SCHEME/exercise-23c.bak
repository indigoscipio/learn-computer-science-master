...
(define (my-vector-fill! v x)
  (let ((vl (vector-length v)))
    ...
    (define (my-vector-fill-helper vect item i)
      (cond [(< i 0) vect]
            [else (begin (vector-set! vect i item)
                         (my-vector-fill-helper vect item (- i 1))
                         )])
      )
    (my-vector-fill-helper v x (- vl 1))
...
(define (vector-append vector1 vector2)
  (let* ((v1-length (vector-length vector1))
        (v2-length (vector-length vector2))
        (new-vect (make-vector (+ v1-length v2-length) 0)))
    
    (define (copy-v1! v1 i1)
      (cond [(< i1 0) 'done]
            [else (let ((curr-item (vector-ref v1 i1)))
                    (begin (vector-set! new-vect i1 curr-item)
                           (copy-v1! v1 (- i1 1) ))
                    )]
            ))

    (define (copy-v2! v2 i2)
      (cond [(< i2 0) 'done]
            [else (let ((curr-item (vector-ref v2 i2)))
                    (begin (vector-set! new-vect (+ i2 v1-length) curr-item)
                           (copy-v2! v2 (- i2 1) ))
                    )])
      )


    (begin (copy-v1! vector1 (- v1-length 1))
           (copy-v2! vector2 (- v2-length 1))
           new-vect)
    
    )
  )
;(vector-append '#(not a) '#(second time)) ;should return #(not a second time)
...
(define (my-vector->list vect)
  ;for each item in vect, store it in temp var first
  ; then cons it to a list, recurse until it reaches the last item
  (define (helper v i)
    (cond [(>= i (vector-length vect)) '()]
          [else (let ((curr-item (vector-ref v i)))
                  (cons curr-item (helper v (+ i 1)))
                  )])
    )
  (helper vect 0)
 
  )
(my-vector->list (vector 1 2 3 4 5))
