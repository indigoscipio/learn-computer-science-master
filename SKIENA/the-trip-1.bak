...
(define (3np1 i j)
  ...
  (define end (max i j))
  ;(list i j (apply max (map (lambda (item) (cycle-length item 1) ) (range start (+ end 1)))))

  ; using fold
  ; in each iteration, compare curr max to previous max
  ; [Number Number -> Number] Number -> Number
  (foldr (lambda (n acc) (max (cycle-length n 1) acc) ) 1 (range start (+ end 1)))
