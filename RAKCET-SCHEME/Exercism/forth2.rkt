...
(define dictionary '(("square" ("dup" "*"))("double" ("dup" "+"))))
  ...
  (define (iter l acc)
    (cond [(null? l) (reverse acc)]
          [(string=? (car l) ";") (let ((definition (list (car (reverse acc)) (cdr (reverse acc))))
                                    ...
                                    (list remaining-tokens (append (list definition) dict))
      ...
      (cond [(null? tokens) stack] ;nothing to evaluate
            [else (let ((token (car tokens)))
                    (cond       
                      ...
                      [(assoc token dict) 
                       (let* ((definition (cadr (assoc token dict)))
                              (expanded-tokens (append definition (cdr tokens))))
                         (evaluate expanded-tokens stack dict))]
