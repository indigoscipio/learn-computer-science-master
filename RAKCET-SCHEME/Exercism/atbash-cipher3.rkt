...
(define (encode phrase)
  (let* ((chars (map char-downcase (string->list phrase)))
         ...
         (encoded-chars (map (lambda (char) (if (char-alphabetic? char)
                                                (encode-char char)
  ...
  |#
    ...
    (string-join (map list->string (iter encoded-chars '())))

    ; approach 3, using for/list
    (string-join (map list->string (for/list ((i (in-range 0 (length encoded-chars) 5)))
                                     (grab-first 5 (drop-first i encoded-chars))
                                     )))
