...
;; Number List-of-Any -> List-of-List-of-Any
;; chunk n l => list of sublists of size n (last may be shorter)
(define (chunk num list)
    (define (iter n l acc)
...
(define (encode key text)
  (let* ((chars (map char-downcase (string->list text)))
         ...
         (encrypted-chars (map (lambda (char) (if (char-numeric? char)
                                                  char
    ...
    (if is-key-valid?
        (string-join (map list->string (chunk 5 encrypted-chars ) ))
