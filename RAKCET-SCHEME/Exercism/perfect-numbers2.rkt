...
(define (add-factors factors counter partner n)
  (cond [(= counter n) factors]           ; Don't add the number itself
        [(= partner n) (cons counter factors)]  ; Only add counter if partner equals n
        [(not (= counter partner)) (append (list counter) (list partner) factors)]
        [else (cons counter factors)]))
