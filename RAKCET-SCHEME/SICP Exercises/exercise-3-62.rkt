...
(define (integrate-series s)
  ; on each iteration, increase the division of coefficient by 1/n+1 and increase power by n+1
  ; counter represents the division 1/counter
  (define (iter stream counter)
    (cons-stream (/ (stream-car stream) counter) (iter (stream-cdr stream) (add1 counter) ) )
    )
  ;initialze iter with counter set to 1
  (iter s 1)
  )

; part b
; deriv of sin(x) is cos(x)
; deriv of cos(x) is -sin(x)
; if you integrate cos(x), you get sin(x)+ a constant
; if you integrate -sin(x), you get cos(x)+ a constant
...
(define (mul-series s1 s2)
  (cons-stream (* (stream-car s1) (stream-car s2)) (add-streams (scale-stream (stream-car s1) (stream-cdr s2))
...
(define (invert-unit-series s)
  (cons-stream 1 (stream-map - (stream-mul (stream-cdr s) (invert-unit-series s))) )
  )
...
;Exercise 3.62
; divides s1 by s2
; s2 is the denuminator and cannot be 0
(define (div-series s1 s2)
  (cond [(= (stream-car s2) 0) (error "Denuminator cannot be 0!" (stream-car s2))]
