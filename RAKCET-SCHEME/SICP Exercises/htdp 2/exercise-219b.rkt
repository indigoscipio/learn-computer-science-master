...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname exercise-219b) (read-case-sensitive #t) (teachpacks ((lib "convert.rkt" "teachpack" "htdp") (lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "convert.rkt" "teachpack" "htdp") (lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp")) #f)))
;Constants
(define GRID-SIZE 5)
...
[(equal? (snake-d (world-snake ws)) "left")
     (if (equal? (make-posn (- (posn-x (snake-h (world-snake ws))) GRID-SIZE)
                              (posn-y (snake-h (world-snake ws))))
                 (world-food-pos ws))
         ; Food eaten: update head, add old head to tail (grow)
         (make-world 
          (make-snake (make-posn (- (posn-x (snake-h (world-snake ws))) GRID-SIZE)
                                 (posn-y (snake-h (world-snake ws))))
                      "left"
                      (cons (snake-h (world-snake ws)) 
                            (snake-t (world-snake ws))))
          (food-create (make-posn (- (posn-x (snake-h (world-snake ws))) GRID-SIZE)
                                  (posn-y (snake-h (world-snake ws))))))
         ; Normal move: update head, add old head to tail and remove last tail element
         (make-world 
          (make-snake (make-posn (- (posn-x (snake-h (world-snake ws))) GRID-SIZE)
                                 (posn-y (snake-h (world-snake ws))))
                      "left"
  ...
  (cond  #|[(equal? (snake-h (world-snake ws)) (world-food-pos ws))
    ...
    [(equal? (snake-d (world-snake ws)) "left")
     (if (equal? (make-posn (- (posn-x (snake-h (world-snake ws))) GRID-SIZE)(posn-y (snake-h (world-snake ws)))) (world-food-pos ws))
         ; food eaten?
    ...
    ;food not eaten
     (make-world (make-snake (make-posn (+ (posn-x (snake-h (world-snake ws))) GRID-SIZE) (posn-y (snake-h (world-snake ws))))
                             "right"
                             (if (empty? (snake-t (world-snake ws)))
                                 '()
