...
(define (edit ed ke)
  ...
  (if (= (string-length ke) 1)
      ...
      ;if it's 1string, check for the following
       (cond
         ; if it's backspace, i tdeletes the character to the left of the cursor (if theres any)
         [(and (string=? "\b" ke) (>= (string-length (editor-pre ed)) 1) ) (make-editor (string-remove-last (editor-pre ed)) (editor-post ed))]
