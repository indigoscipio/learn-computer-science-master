...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname exercise-207) (read-case-sensitive #t) (teachpacks ((lib "convert.rkt" "teachpack" "htdp") (lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "convert.rkt" "teachpack" "htdp") (lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp")) #f)))
...
(define LLISTS (read-itunes-as-lists ITUNES-LOCATION))
; LLists example
; An LLists is one of:
; – '()
; – (cons LAssoc LLists)
...
; find-association consumes String, LAssoc, and an element of Any called default.
...
;(find-association "Genre" (first LLISTS) "No KV pair found")


; total-time/list consumes LLists and produces the total amount of play time
; LLists -> Number
(define (total-time/list llists)
  (cond [(empty? llists) 0]
        [else (+ (second (find-association "Total Time" (first llists) 0)) (total-time/list (rest llists))) ])
  )


(define SAMPLE-LASSOC (list
 (list "Track ID" 442)
 (list "Name" "Wild Child")
 (list "Completed" #true)
 (list "Artist" "Enya") 
...
;is-value-boolean takes an Assoc and check if the value is a boolean
(define (is-value-boolean? assoc)
...
;boolean-attributes-lassoc processes LAssoc and outputs a list of string of Assoc pair which value is a boolean
; LAssoc -> Los
(define (boolean-attributes-lassoc lassoc)
  (cond [(empty? lassoc) '()]
        [(is-value-boolean? (first lassoc)) (cons (first (first lassoc)) (boolean-attributes-lassoc (rest lassoc)) )]
        [else (boolean-attributes-lassoc (rest lassoc))])
  )
;(boolean-attributes-lassoc (rest lassoc))
(boolean-attributes-lassoc SAMPLE-LASSOC)


;boolean-attributes consumes an LLists and produces the string that are associated with a Boolean attribute
; LLists -> LoS
