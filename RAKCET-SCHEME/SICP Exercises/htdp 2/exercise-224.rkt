...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname exercise-224) (read-case-sensitive #t) (teachpacks ((lib "convert.rkt" "teachpack" "htdp") (lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp"))) (htdp-settings #(#t constructor repeating-decimal #f #t none #f ((lib "convert.rkt" "teachpack" "htdp") (lib "image.rkt" "teachpack" "2htdp") (lib "universe.rkt" "teachpack" "2htdp") (lib "batch-io.rkt" "teachpack" "2htdp") (lib "itunes.rkt" "teachpack" "2htdp")) #f)))
(define GRID-SIZE 5)
...
(define sample-ufo (make-posn 25 25))
(define sample-ufo2 (make-posn 95 95))
...
; space invader game
(define sample-si (make-si sample-ufo sample-tank missile-fired1 ))
(define si-init (make-si sample-ufo sample-tank missile-empty))

; UFO -> Image
(define (render-ufo ufo img)
  (place-image UFO-IMG (posn-x ufo) (posn-y ufo) img )
  )
;(render-ufo sample-ufo2 SCENE)
...
;(place-image MISSILE-IMG (posn-x (first missile-fired2)) (posn-y (first missile-fired2))
;             (place-image MISSILE-IMG (posn-x (second missile-fired2)) (posn-y (second missile-fired2)) SCENE) ) 
...
(define (move-ufo ufo)
  (make-posn (posn-x ufo) (+ (posn-y ufo) UFO-SPEED) )
  )
(move-ufo sample-ufo)
...
;checks if theres one from the given list of missile that hits ufo
...
(missiles-hit-ufo? missile-fired3 sample-ufo)
...
(define (si-end? ws)
  ...
  (cond [(>= (posn-y (si-ufo ws)) (- SCENE-HEIGHT GRID-SIZE)) #true]
        ;check if missile hits ufo 
        [(missiles-hit-ufo? (si-missile ws) (si-ufo ws)) #true ]
        [else #false])
  )

(define (si-render-game-over ws)
  (if (si-end? ws)
      (place-image (text "Game Over!" 16 "red") (/ SCENE-WIDTH 2) (/ SCENE-HEIGHT 2) (si-render ws))
      (si-render ws)
      )
  )
