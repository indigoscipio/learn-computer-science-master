...
; applies procedure p for each item in l
(define (my-for-each.v1 p l)
  (cond [(empty? l) #t]
        [else (p (first l))  (my-for-each.v1 p (rest l)) ])
  )
(my-for-each.v1 (lambda(x)(newline)(display x)) '(1 2 3 4))