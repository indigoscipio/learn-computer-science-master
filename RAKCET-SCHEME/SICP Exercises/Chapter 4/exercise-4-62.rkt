...
; Exercise 4.59
; in previous chapter append is defined as a function
; in query append works as a relations - it work bidirectionally
...
(rule (?x next-to ?y in (?x ?y . ?u))) ;empty body
(rule (?x next-to ?y in (?v . ?z))
      (?x next-to ?y in ?z))

; what will be the response be to the folowing queries?
; answer:
(?x next-to ?y in (1 (2 3) 4))
; we want to find ?x and ?y in (1 (2 3) 4)
; this generates all possibilieis of number next to each other
; (2,3) next to 1, 4 next to (2,3)

(?x next-to 1 in (2 1 3 1))
; we want to find ?x before 1
; (2,1) -> 2, (3,1) -> 3

; =============================

; Exercise 4.62
; returns a list containing the last element of a nonempty lisy
(rule (last-pair (?x) (?x))
      ; base case: if the list contains 1 item left, return it
      )
(rule (last-pair (?first . ?rest) ?last-pair )
      ; otherwise, recurse and reduce the rest of the list
