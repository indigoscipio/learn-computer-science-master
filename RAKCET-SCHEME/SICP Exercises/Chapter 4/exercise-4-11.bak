...
(define (set-variable-value! var val env)
  ; scan for the variable just as in lookup variable value
  (define (scan variables values)
    (cond [(or (null? variables) (null? values)) #f]
...
(define (define-variable! var val env)
  ; search first frame for a binding for the variable
  ; and change the binding if it exists (just as in set-variable-value!)
  ; if no bindings exists, we adjoin one to the first frame
  (let ((frame (first-frame env)))
    ; Variables Values -> MaybeValue
    (define (scan variables values)
      (cond [(null? variables) (add-binding-to-frame! var val frame)]
            [(eq? var (car variables)) (set-car! values val)]
            [else (scan (cdr variables) (cdr values))]
            )
      )
