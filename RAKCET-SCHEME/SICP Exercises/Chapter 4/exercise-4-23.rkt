...
; answer:
; Alyssa's version
; alyssa maps analyze over a list of expressiosn -> gets a list of execution procedures -> wraps them in execute-sequence
; the logic (do this, do that etc) isn't built here -> it gets build at execution time (execute-sequence ...) by traversing through the list
; in execution time, execute sequence will do proc1, recurse, do proc2, recurse, etc
; for exery expression this executes the procedure at execution time (loop happening at execution/runtime)
...
; takes two execution procedure in order but the execution procedure is built at analysis time
; the sequence logic is already stored into the lambda when sequentially is run
; so it builds a list of big chained lambdas which is more optimized version of alyssa
