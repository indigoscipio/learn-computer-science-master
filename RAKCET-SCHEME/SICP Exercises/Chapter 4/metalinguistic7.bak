...
; Exercise 4.20a
; implement letrec as a derived expression by transforming letrec into let
; letrec vars should be created with a let and assigned their values with set!

;; letrec->let : Expression -> Expression
(define (my-letrec exp)
  
  (define (build-new-bindings bindings)
    (map (lambda (binding) (cons (car binding) '*unassigned*) ) bindings)
    )

  (define (build-set!-expr bindings)
  ...
  (let* ((body body-exprs)
         (old-bindings (cadr exp))
         (new-bindings (build-new-bindings old-bindings))
         )

    (list 'let new-bindings (append (build-set!-expr old-bindings) body) )
    )
  )
...
; env diagram with letrec (original)
; global env: (f x)
; calls for (f 5)
; creates a new frame E1, and within E1:
; binds even? to *unassigned* and odd? to *unassigned*
; evaluates set! expression to assign them to their lambda values. They are now usable
; sees the body expression (even? x) and evaluates it, creates a new frame E2 and binds x = 5 which is from E1

; evaluates even? within E1
; sees that even? references odd? - gets the value since both are already defined
; the mutual recursion works and the value returnd -> false
...
;env diagram with let in place of letrec in the definition of f

; global env: (f.v2 x)
; calls for (f.v2 5)
; creates new frame E1 that binds x = 5
; but before we create the frame e1, sceheme evaluates the value of the lambda
; unlike letrec, the value lambda procedure of even? and odd? is not unassigned
; sees that even? reference to odd? and calls for (odd? ...)
; returns a error for unbound variable, the remaiing code will not run
...
; answer:
; let's try to analyze this and see if we can confirm if the value returned is 10!
; normally we define a recursive function (iter n) and call for it
; but this is an unnamed application of (lambda (n)) with 10 as the argument
; let's call this outer lambda A 
; creates a new frame E1 -> binds n = 10

; inside this lambda procedure, we call for another lambda application
; a new frame E2 is ccreated
; we're calling (lambda (fact) (fact fact n)) with argument (lambda (ft k) (if (= k 1) 1 (* k (ft ft (- k 1)))))
; evaluates (lambda (fact) (fact fact n)) -> binds fact to (if (= k 1) 1 (* k (ft ft (- k 1)))), which is the argument
; then evaluate the body (fact fact n) = ((lambda (ft k) ... ) fact n )
; binds fact = ft and n = k = 10
; then it starts the recursion and evaluates the logic (if 10 = 1 return 1, else (* 10 (fact fact 9) ))

; 2nd recursion: evaluate (ft ft 9)
; (if (= 9 1) 1 (* 9 (ft ft (- 9 1))))
; do this continually until it reaches the base case = 1, evaluate the previous acucmulated value, and we get 10! return value

; 2nd part: fibonacci version with pure unnamed lambda
;; Nat -> Nat
...
((lambda (n)
   ((lambda (fib) (fib fib 1 0 n))
    (lambda (f a b count) (if (= count 0) b (f f (+ a b) a (- count 1) ))))
   ) 10)

; Exercise 4.21b


; if its true, call odd with n - 1
; if its odd, call even with n - 1
(define (f x)
  (; takes even? and odd? -> returns even? even? odd? x 
...
; evaluation analysis
; (f 4) application, binds x = 4
; lambda that takes 2 argument (even? and odd?) and returns (even? even? odd? x)
; binds even? to (lambda (ev? od? n)(if (= n 0) true (od? ev? od? (- n 1)) ))
; binds odd? to (lambda (ev? od? n)(if (= n 0) false (ev? ev? od? (- n 1))))

; evaluates (even? even? odd? x)
; ((lambda (ev? od? n)(if (= n 0) true (od? ev? od? (- n 1)) )) even? odd? x)