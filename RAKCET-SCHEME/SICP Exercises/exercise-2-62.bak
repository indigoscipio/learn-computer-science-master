...
; Number Set -> Set
; adjoin-set using ordered representation
(define (adjoin-set.v3 x set)
  (cond [(empty? set) (list x)]
        [(= x (car set)) set ]
        [(< x (car set)) (cons x set) ]
        [else (cons (car set) (adjoin-set.v3 x (cdr set)))] ; otherwise insert it set
        )
  )


; element-of-set? using ordered representation
; Number Set (Ordered) -> Boolean
(define (element-of-set?.v4 x set)
  (cond [(empty? set) #f]
        [(= x (car set)) #t]
        [(< x (car set)) #f]
        [else (element-of-set?.v4 x (cdr set))]
        )
  )

; exercise 2.62
; O(n) implementation of union-set for sets represented as ordered list
; Set (Ordered) Set(Ordered) -> Set(Ordered)
(define (union-set.v3 set1 set2)
  (cond [(empty? set1) set2]
        [(empty? set2) set1]
        [(< (car set1) (car set2)) (cons (car set1) (union-set.v3 (cdr set1) set2))] ;build the union in set2
        [(< (car set2) (car set1)) (cons (car set2) (union-set.v3 set1 (cdr set2)))]
        [(= (car set1) (car set2)) (cons (car set1) (union-set.v3 (cdr set1) (cdr set2)))] ; else car set1 = car set1, keep one and reduce both
        )
  )
(union-set.v3 '(1 2 3 4) '(5 6 7 8))
(union-set.v3 '(1 1 1 3) '(1 2 3 4))
(union-set.v3 '() '())
(union-set.v3 '() '(4 5 6))
(union-set.v3 '(4 5 6) '())