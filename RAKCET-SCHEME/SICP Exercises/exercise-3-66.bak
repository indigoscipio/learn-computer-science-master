...
; answer:
; in the original sqrt stream, we're storing guesses as a variable and everytime sqrt stream is run guess is returned
; so there's no need for computation everytime sqrt stream is called

; in louis's program, it doesn't store the cons stream in a variable
; so everytime sqrt-stream is called, it recomputes cons stream unecessarily form scratch

; and yes, the two versions will still differ in efficiency if we didn't use the memoize procedure
; because we need to recompute the current guess and the next one and the next one so it grows exponentially
...
(define (stream-limit s tolerance)
  (let ((first (stream-car s))
    ...
    (cond [(< difference tolerance) second]
